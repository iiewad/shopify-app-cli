  ...
  module Result
    class WrapTest < Minitest::Test
      def test_wraps_exceptions_in_an_error_result
        assert Result.wrap(RuntimeError.new).error?
      end

      def test_wraps_values_in_a_success_result
        assert Result.wrap("Success").success?
      end

      def test_does_not_wrap_another_result_again
        assert_equal "Success", Result.wrap(Result.wrap("Success")).unwrap
      end

      def test_supports_deferring_result_construction
        assert_equal "Success", Result.wrap { "Success" }.call.unwrap
      end

      def test_forwards_caller_argument_when_deferring_result_construction
        assert_equal "Success", Result.wrap { |value| value }.call("Success").unwrap
      end

      def test_captures_exceptions_and_wraps_them_in_an_error_when_deferring_result_construction
        assert Result.wrap { raise "Error" }.call.error?
      end
    end
